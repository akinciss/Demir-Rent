 ...
 *
 * Admin aktif bir rezervasyonu iptal eder (aktif → iptal).
 * Transaction içinde:
 *  1. Admin token + rol doğrulama
 *  2. Rental var mı, aktif durumunda mı?
 *  3. Rental status → "iptal"
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      // Slot'u tekrar available yap (iptal durumunda slot serbest bırakılır)
      if (rental.slotId) {
    ...
    console.error("Admin iptal transaction hatası:", err);
    return NextResponse.json({ error: "İptal işlemi sırasında beklenmeyen bir hata oluştu." }, { status: 500 });
  }
}
